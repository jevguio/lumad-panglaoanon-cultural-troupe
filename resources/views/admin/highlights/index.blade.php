    ...
    <div class="grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:10px;">
        @foreach($showEvents as $event)
            <div style="border:1px solid #ccc;padding:10px;border-radius:6px;">
                ...
                <button onclick="openHighlightModal({{ $event->id }}, {{ $event->highlights->first()->id ?? 'null' }})" style="background:#4CAF50;color:white;border:none;padding:4px 8px;margin-top:5px;">EDIT</button>
            </div>
        @endforeach
    </div>
</div>
...
<!-- MODAL -->
<div id="highlightModal" class="modal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);justify-content:center;align-items:center;z-index:1000;">
  <div style="background:white;padding:20px;border-radius:8px;width:500px;position:relative;max-height:90vh;overflow:auto;">
    <button type="button" onclick="closeHighlightModal()" style="position:absolute;top:10px;right:10px;background:none;border:none;font-size:18px;cursor:pointer;">✖</button>
    ...
    <form id="highlightForm" method="POST" enctype="multipart/form-data">
      ...
      <input type="number" name="event_id" id="eventIdField" required style="width:100%;padding:5px;margin-bottom:10px;">
      
      <label>Image Media</label>
      <input type="file" name="images[]" id="imageInput" multiple accept="image/*" style="width:100%;margin-bottom:10px;">
      <div id="imagePreview" style="display:flex;flex-wrap:wrap;gap:5px;"></div>

      <label>Video Media</label>
      <input type="file" name="videos[]" id="videoInput" multiple accept="video/*" style="width:100%;margin-bottom:10px;">
      ...
      <div style="margin-top:10px;">
        ...
        <button type="button" onclick="closeHighlightModal()" style="background:#ccc;border:none;padding:6px 10px;">CANCEL</button>
      </div>
    </form>
  </div>
</div>
